...
import { extractParameters, getParamNames, isZodSchemaOptional } from "@server/validation";
...
export function Tool(description?: string): MethodDecorator {
  return (target: object, propertyKey: string | symbol, descriptor: PropertyDescriptor) => {
    ...
    // 获取参数类型信息
    const _paramTypes = (Reflect.getMetadata("design:param:types", target, propertyKey) as unknown[]) || [];
...
export function Param(zodSchema: z.ZodType<unknown>, description?: string): ParameterDecorator {
  return (target: object, propertyKey: string | symbol | undefined, parameterIndex: number) => {
    ...
    // 获取参数的运行时类型信息
    const paramTypes = Reflect.getMetadata("design:param:types", target, actualPropertyKey) || [];
